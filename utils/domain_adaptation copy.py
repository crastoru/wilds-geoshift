...
class DomainAdaptationLossAdjustment:
    """
    Base class for various methods of adjusting the loss function for improved robustness to subpopulation shifts.
    Subclasses should implement the `loss` method, which takes in the necessary parameters and returns the modified loss.
    """
    ...
    def update_state(self, *args, **kwargs) -> None:
        """
        Update the internal state of the loss adjustment method.
        This method can be overridden by subclasses to implement specific state updates.
        """
        pass
...
class GroupDRO(DomainAdaptationLossAdjustment):
    """
    Implements Group Distributionally Robust Optimization (Group DRO) for domain adaptation.
    It reweights the loss based on group-specific losses to minimize the worst-case loss across groups.
    """
    def __init__(self, n_groups, step_size=0.01, device="cpu"):
        super().__init__(device)
        self.n_groups = n_groups
        self.group_weights_step_size = step_size
        self.group_weights = torch.ones(n_groups).to(device) / n_groups

    def loss(self, groups, losses):        
        group_losses = torch.zeros(self.n_groups).to(self._device)
        for i in range(self.n_groups):
            if any(groups == i):
                group_losses[i] = torch.mean(losses[groups == i])
        
        self.update_state(group_losses.detach())
        loss = self.group_weights @ group_losses
        return loss

    def update_state(self, group_losses):
        self.group_weights = self.group_weights * torch.exp(self.group_weights_step_size*group_losses)
        self.group_weights = self.group_weights/self.group_weights.sum()



class CORAL(DomainAdaptationLossAdjustment):
    def __init__(self, penalty_weight, device="cpu"):
        super().__init__(device)
        self.penalty_weight = penalty_weight

    def loss(self, groups, features, losses):
        penalty = coral_loss(groups, features, self._device)
        return torch.mean(losses) + self.penalty_weight * penalty
    

class IRM(DomainAdaptationLossAdjustment):
    def __init__(self, penalty_weight, device="cpu"):
        super().__init__(device)
        self.penalty_weight = penalty_weight

    def loss(self, groups, losses, logits, y, loss_fn):
        penalty = irm_penalty(groups, logits, y, loss_fn, self._device)
        return torch.mean(losses) + self.penalty_weight * penalty
...
def get_domain_adaptation_loss_adjustment(name, *args, **kwargs):
    name = name.lower()
    if name not in DOMAIN_ADAPTATION_METHODS:
        raise ValueError(f"Unknown domain adaptation method: {name}")
    return DOMAIN_ADAPTATION_METHODS[name](*args, **kwargs)
